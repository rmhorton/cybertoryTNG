...
class TrashCan {
  constructor(x, y) {
    ...
    this.lid = this.group.select('.trash-lid'); // selectAll
  }
  ...
  onDragEnd(ev) {
    this.group.transition().duration(10).attr('transform', `translate(${this.tx},${this.ty})`);
  }
  ...
  lid_movement() { // I couldn't figure out how to set a static data member; the get method didn't seem to work.
    return 25;
  }
  ...
  openLid(){ // !!!
    ...
    if (!this.isLidOpen) {
      ...
      this.lid.transition().duration(10).attr('transform', `translate(0, ${-LID_MOVEMENT})`);
      this.isLidOpen = true;
    }
  }
